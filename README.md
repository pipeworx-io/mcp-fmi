...
`tools/list` at `https://gateway.pipeworx.io/fmi/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/fmi_forecast \
  -H 'Content-Type: application/json' \
  -d '{"place":"Helsinki"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/fmi_forecast`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
